 ...
 *
 * These BSTs are  balanced so worst-case all operations except
 * size() are O(logn). Each iterator operates on a copy of the keys, so
...
public class AvlTreeMap<K extends Comparable<? super K>, V>
    ...
    private int bf(Node n) {
        ...
        return this.ht(n.left) - this.ht(n.right);
    }

    //this function makes sure the tree is balanced
    private Node balanced(Node n) {
        if (this.bf(n) >= -1 && this.bf(n) <= 1) {
            return n;
        }
        else if (this.bf(n) < 0) {
            if (this.bf(n.right) > 0) {
                return this.doubleRL(n);
            }
            else {
                return this.singleLeft(n);
            }
        }
        else {
            if (this.bf(n.left) < 0) {
                return this.doubleLR(n);
            }
            else {
                return this.singleRight(n);
            }
        }
    }
    ...
    //update heights
    private Node updateHL(Node n) {
        if (n.left == null) {
            if (n.right == null) {
                n.height = 0;
            }
            else if (ht(n.right) + 1 != ht(n)) {
                n.height--;
            }
        }
        else if (ht(n.left) + 2 == ht(n) && ht(n.right) + 1 != ht(n)) {
            n.height--;
        }
        return n;
    }
    ...
    //updates the Height
    private Node updateHR(Node n) {
        if (n.right == null) {
            if (n.left == null) {
                n.height = 0;
            }
            else if (ht(n.left) + 1 != ht(n)) {
                n.height--;
            }
        }
        else if (ht(n.right) + 2 == ht(n) && ht(n.left) + 1 != ht(n)) {
            n.height--;
        }
        return n;
    }
    ...
    //NEED TO FIND A WAY TO CHANGE THE HEIGHTS OF THE PARENT NODES
    ...
    //this function performs a doubleRightLeft rotation
    private Node doubleRL(Node n) {
        n.right = singleRight(n.right);
        return singleLeft(n);
    }

    //this function performs a doubleLeftRight rotation
